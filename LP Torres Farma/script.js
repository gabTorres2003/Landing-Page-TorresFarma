document.addEventListener('DOMContentLoaded', function() {
    ...
    if (carrosselContainer) {
        ...
        const produtosGrid = carrossel ? carrossel.querySelector('.produtos-grid') : null;

        if (produtosGrid && produtosGrid.children.length > 0 && btnPrev && btnNext) {
    ...
    const carrinhoQuantidadeIcone = document.querySelector('.carrinho-quantidade');
    ...
    function atualizarInterfaceCarrinho() {
        if (!carrinhoItemsContainer || !carrinhoQuantidadeIcone || !carrinhoTotalDisplay) {
            return;
        }
    ...
    function adicionarProdutoAoCarrinho(nome, preco, imagem, mostrarFeedback = true) {
    ...
    function aumentarQuantidadeItem(index) {
        adicionarProdutoAoCarrinho(carrinho[index].nome, carrinho[index].preco, carrinho[index].imagem, false);
    }
    ...
    if (carrinhoItemsContainer) {
        carrinhoItemsContainer.addEventListener('click', function(event) {
            ...
            if (target.classList.contains('carrinho-item-diminuir') || target.closest('.carrinho-item-diminuir')) {
                diminuirQuantidadeItem(index);
            } else if (target.classList.contains('carrinho-item-aumentar') || target.closest('.carrinho-item-aumentar')) {
                aumentarQuantidadeItem(index);
            } else if (target.classList.contains('carrinho-item-remover-total') || target.closest('.carrinho-item-remover-total')) {
    ...
    if (btnFinalizarCompra) {
        btnFinalizarCompra.addEventListener('click', function() {
            ...
            let subTotalPedido = 0;
            const itensParaMensagem = carrinho.map(item => {
                const totalItem = item.preco * item.quantidade;
                subTotalPedido += totalItem;
                return `${item.quantidade}x ${item.nome} (R$ ${item.preco.toFixed(2)} cada) - Total Item: R$ ${totalItem.toFixed(2)}`;
            }).join("\n");
            
            const mensagemWhatsapp = "Olá, Drogaria Torres Farma! Gostaria de fazer o seguinte pedido:\n" +
                                    itensParaMensagem +
            ...
            if (numeroFarmacia.length < 10) { 
                 alert("Número de WhatsApp da Farmácia se encontra desconfigurado!");
                 return;
            }

            alert('Você será redirecionado para o WhatsApp para finalizar seu pedido. Seu carrinho no site será esvaziado.');
