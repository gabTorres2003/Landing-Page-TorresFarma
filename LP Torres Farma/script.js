document.addEventListener('DOMContentLoaded', function() {
    ...
    // Clona os últimos cards originais e os insere no início da grade.
    originalCards.slice().reverse().forEach(card => {
        produtosGrid.insertBefore(card.cloneNode(true), produtosGrid.firstChild);
    });
    // Clona os primeiros cards originais e os insere no final da grade.
    ...
    function calculateDimensions() {
        ...
        } // senão, mantém o valor anterior de cardWidth
    ...
    // Função principal para mover o carrossel e gerenciar o "salto"
    function moveTo(newIndex) {
        if (isTransitioning) return;
        isTransitioning = true;

        currentIndex = newIndex; // Define o novo índice alvo para a animação
        ...
        setTimeout(() => {
            let jumped = false;
            // Chegou nos clones do final (que são cópia do início)?
    ...
    function updateButtonVisibility() {
        const canScroll = numOriginalCards > 0 && produtosGrid.scrollWidth > Math.ceil(carrossel.clientWidth) + 1;
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  function atualizarCarrinho() {
    ...
    carrinho.forEach((item, index) => {
      ...
      itemElement.innerHTML = `
        ...
        <button class="carrinho-item-remover" data-index="${index}" aria-label="Remover ${item.nome} do carrinho">
  ...
  document.addEventListener('click', function(event) {
    if (!carrinhoDropdown.contains(event.target) && !carrinhoBtn.contains(event.target)) {
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  function updateCarrossel() {
    ...
    const gap = parseFloat(getComputedStyle(document.querySelector('.produtos-grid')).gap) || 24; // 1.5rem em pixels
    ...
    } else if (produtosGrid.length > 1) { // Se há múltiplos .produtos-grid como slides
  ...
  if (btnNext && btnPrev) { // Só adiciona listeners se os botões existem
    btnNext.addEventListener('click', () => {
      ...
      if (produtosGrid.length === 1 && carrossel.contains(produtosGrid[0])) {
          ...
          const cardWidthWithGap = document.querySelector('.produto-card').offsetWidth + (parseFloat(getComputedStyle(document.querySelector('.produtos-grid')).gap) || 24);
          const visibleCards = Math.floor(carrossel.clientWidth / cardWidthWithGap);
          if (currentIndex < numCards - visibleCards) { // Permite rolar até o último card ficar visível
              currentIndex++;
              updateCarrossel();
          }
      } else if (produtosGrid.length > 1) { // Múltiplos .produtos-grid como slides
  ...
  function checkButtons() {
    ...
    btnNext.style.visibility = carrossel.scrollLeft >= maxScrollLeft - 5 ? 'hidden' : 'visible';
    ...
    } else if (produtosGrid.length > 1) { // Múltiplos .produtos-grid
        btnPrev.style.visibility = currentIndex === 0 ? 'hidden' : 'visible';
        btnNext.style.visibility = currentIndex === totalSlides - 1 ? 'hidden' : 'visible';
    }
  }
  
  if (carrossel) { // Só executa se o carrossel existir
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  // Função para atualizar o carrinho na interface
  function atualizarCarrinho() {
    if (!carrinhoItemsContainer || !carrinhoQuantidade || !carrinhoTotal) return; // Proteção
    ...
    document.querySelectorAll('.carrinho-item-aumentar').forEach(btn => {
      btn.addEventListener('click', function() {
        ...
        adicionarAoCarrinho(carrinho[index].nome, carrinho[index].preco, carrinho[index].imagem, false); // false para não mostrar feedback
      });
    });
  ...
  function adicionarAoCarrinho(nome, preco, imagem, mostrarFeedback = true) {
    ...
    if (mostrarFeedback) {
      ...
      setTimeout(() => {
        feedback.classList.remove('show');
        setTimeout(() => {
          if (document.body.contains(feedback)) { // Verifica se o elemento ainda existe
             document.body.removeChild(feedback);
          }
        }, 300);
      }, 2000);
  ...
  if(btnFinalizar){
      btnFinalizar.addEventListener('click', function() {
        ...
        const mensagemWhatsapp = "Olá, gostaria de finalizar minha compra com os seguintes itens:\n" +
                                carrinho.map(item => `${item.quantidade}x ${item.nome} - R$ ${(item.preco * item.quantidade).toFixed(2)}`).join("\n") +
                                "\nTotal: " + carrinhoTotal.textContent;
        const numeroFarmacia = "SEU_NUMERO_WHATSAPP_COMPLETO_SEM_SIMBOLOS_EX:5522999999999"; 
       
        const mensagemCodificada = encodeURIComponent(mensagemWhatsapp);
        
        window.open(`https://wa.me/${numeroFarmacia}?text=${mensagemCodificada}`, '_blank');

        alert('Você será redirecionado para o WhatsApp para finalizar sua compra. Se preferir, pode limpar o carrinho após confirmar o pedido.');
      });
  }
  ...
  document.addEventListener('click', function(event) {
    if (carrinhoDropdown && !carrinhoDropdown.contains(event.target) && carrinhoBtn && !carrinhoBtn.contains(event.target)) {
      carrinhoDropdown.classList.remove('active');
    }
  });
